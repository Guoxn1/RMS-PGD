...
class PSOAttacker(BaseAttacker):
    def __init__(self, obs_length, pred_length, attack_duration, predictor, n_particles=10, iter_num=100, c1=0.5, c2=0.3, w=1.0, bound=1, physical_bounds={}):
        ...
        self.options = {'c1': c1, 'c2': c2, 'w': w,"k":5,"p":2}
    ...
    def run(self, data, obj_id, **attack_opts):
        ...
        optimizer = ps.single.LocalBestPSO(n_particles=self.n_particles, dimensions=self.perturb_length * 2, options=self.options, bounds=(-np.ones(self.perturb_length * 2) * self.bound, np.ones(self.perturb_length * 2) * self.bound), center=np.zeros(self.perturb_length * 2))
