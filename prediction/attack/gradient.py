...
class GradientAttacker(BaseAttacker):
    def __init__(self, obs_length, pred_length, attack_duration, predictor, iter_num=100, learn_rate=0.1,
                 learn_rate_decay=20, bound=1, physical_bounds={}, smooth=0, seed_num=10):
    ...
    def run(self, data, obj_id, **attack_opts):
        ...
        for seed in range(self.seed_num):
            ...
            for _obj_id in perturbation["value"]:
                perturbation["value"][_obj_id] = Variable(
                    torch.rand(self.obs_length + self.attack_duration - 1, 2).cuda() * 2 * self.bound - self.bound)
            ...
            for i in range(self.iter_num):
                ...
                for _obj_id in perturbation["value"]:
                    ...
                    processed_perturbation[_obj_id] = hard_constraint(data["objects"][_obj_id]["observe_trace"],
                                                                      perturbation["value"][_obj_id], self.bound,
                ...
                if loss.item() < best_loss:
                    best_loss = loss.item()
                    best_perturb = {_obj_id: value.cpu().clone().detach().numpy() for _obj_id, value in
                                    processed_perturbation.items()}
                    best_iter = i
                    best_out = total_out
                ...
                for _obj_id in perturbation["value"]:
                    grad = \
                        torch.autograd.grad(loss, perturbation["value"][_obj_id], retain_graph=False,
                                            create_graph=False,
                                            allow_unused=True)[0]

                    perturbation["value"][_obj_id] = perturbation["value"][_obj_id].detach() - lr * (
                                grad + 0.1 * past_grad)
                    past_grad = grad
