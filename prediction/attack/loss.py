...
def ade(predict_trace, future_trace):
    ...
    return torch.sqrt(torch.sum(torch.square(predict_trace - future_trace)) / predict_trace.shape[0])+4*torch.sqrt(torch.sum(torch.square(predict_trace[-1,:] - future_trace[-1,:])))
