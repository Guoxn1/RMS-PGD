...
class GRIPDataLoader(DataLoader):
    ...
    def preprocess(self, input_data, perturbation, rescale_x=1, rescale_y=1, smooth=False):
        ...
        visible_object_value = np.array([
            np.concatenate((np.array(input_data["objects"][obj_id]["observe_trace"])[self.obs_length-1,:],
                            np.array(input_data["objects"][obj_id]["observe_feature"])[self.obs_length-1,:]), axis=0) for obj_id in visible_object_id_list])
        ...
        for frame_ind in range(self.seq_length):
            ...
            for obj_id, obj in input_data["objects"].items():
                if frame_ind < self.obs_length:
                    feature_data = np.concatenate((np.array(obj["observe_trace"])[frame_ind,:],
                                                   np.array(obj["observe_feature"])[frame_ind,:]), axis=0)
                    existence = obj["observe_mask"][frame_ind]
                else:
                    feature_data = np.concatenate((np.array(obj["future_trace"])[frame_ind-self.obs_length,:],
                                                   np.array(obj["future_feature"])[frame_ind-self.obs_length,:]), axis=0)
        ...
        if perturbation is not None:
            for _obj_id in perturbation["ready_value"]:
                ori_data[0,3:5,:self.obs_length,obj_index[_obj_id]] += torch.transpose(torch.tensor(perturbation["ready_value"][_obj_id]).cuda(), 0, 1)
        if smooth > 0:
    ...
    def postprocess(self, input_data, perturbation, *args):
        ...
        if perturbation is not None:
            for _obj_id in perturbation["ready_value"]:
                input_data["objects"][str(_obj_id)]["perturbation"] = np.array( perturbation["ready_value"][_obj_id])
            ...
            if "loss" in perturbation and perturbation["loss"] is not None:
                ...
                for _obj_id, obj in input_data["objects"].items():
                    if not obj["visible"]:
                        continue
                    observe_traces[_obj_id] = torch.from_numpy(np.array(input_data["objects"][str(_obj_id)]["observe_trace"])).cuda()
                    future_traces[_obj_id] = torch.from_numpy(np.array(input_data["objects"][str(_obj_id)]["future_trace"])).cuda()
