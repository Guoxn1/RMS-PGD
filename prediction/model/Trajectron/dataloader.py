    ...
    def input_data_to_dataframe(input_data, env):
        ...
        for frame_id in range(obs_length):
            for obj_id, obj in input_data["objects"].items():
                ...
                # if obj["static"]:
                #     continue
                data.append([frame_id, int(obj_id), obj["type"], np.array(obj["observe_trace"])[frame_id,:], np.array(obj["observe_feature"])[frame_id,:]])
        ...
        for frame_id in range(pred_length):
            for obj_id, obj in input_data["objects"].items():
                ...
                # if obj["static"]:
                #     continue
                data.append([frame_id+obs_length, int(obj_id), obj["type"], np.array(obj["future_trace"])[frame_id,:], np.array(obj["future_feature"])[frame_id,:]])
        ...
        for d in data:
            ...
            #df_data = pandas.concat([df_data,data_point],ignore_index=True)
