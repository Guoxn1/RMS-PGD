...
class FQAInterface(Interface):
    ...
    def run(self, input_data, perturbation=None, backward=False):
        ...
        if perturbation is not None:
            a = (np.array(perturbation["ready_value"][perturbation["obj_id"]]) / np.max(
                self.dataset.xy_distribution["std"]))
            a = torch.tensor(a).to("cuda:0")
            sources[obj_index_map[perturbation["obj_id"]]][:self.obs_length, :2] += torch.tensor(a)
        ...
        if perturbation is not None:
            ...
            for obj_id, obj_index in obj_index_map.items():
                ...
                future_traces[obj_id] = torch.from_numpy(np.array(input_data["objects"][obj_id]["future_trace"])).cuda()
